...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="makeOrder + sign + send. Supports EVM, Solana, and Tron. Keys used in memory only, never output."
    )
    ...
    if _is_solana_order(data):
        if not args.private_key_sol:
            print("Error: Solana order detected but --private-key-file-sol not provided", file=sys.stderr)
            sys.exit(1)
        ...
        if not args.private_key_tron:
            print("Error: Tron order detected but --private-key-file-tron not provided", file=sys.stderr)
            sys.exit(1)
        from order_sign import sign_order_txs_tron
        signed = sign_order_txs_tron(data, args.private_key_tron)
    else:
        if not args.private_key:
            print("Error: EVM order detected but --private-key-file not provided", file=sys.stderr)
            sys.exit(1)
