...
Signs makeOrder (make-order) response for EVM, Solana, and Tron chains.
...
Usage:
    ...
    # Pipe from make-order
    python3 scripts/bitget-wallet-agent-api.py make-order ... | python3 scripts/order_sign.py --private-key-file /tmp/.pk

Output: JSON array of signed strings → fill txs[].sig, then call send.
  Format varies by chain:
  - EVM regular: 0x-prefixed signed tx hex
  - EVM gasPayMaster msgs[]: JSON string of msgs with sig fields filled
  - Solana: base58 signed transaction
  - Tron: JSON with { signature, txID, raw_data }
  Also supports data.signatures mode (partner API EIP-712 hashes) if present.

Dependencies: Python 3.9+ stdlib + eth_account (EVM and Tron). Solana signing is
...
def sign_order_txs_solana(order_data: dict, private_key_sol: str) -> list[str]:
    ...
    for tx_item in txs:
        ...
        if item_cid is not None and int(item_cid) != _SOLANA_CHAIN_ID:
            raise ValueError(
                f"Tx item has non-Solana chainId {item_cid}. Use --private-key-file for EVM orders."
            )
        ...
        if item_cid is None and chain_field not in ("sol", "solana", ""):
            raise ValueError(
                f"Tx item has non-Solana chain '{chain_field}'. Use --private-key-file for EVM orders."
            )
...
def sign_order_signatures(order_data: dict, private_key: str) -> list[str]:
    """
    Sign all EIP-712 hash signatures in a makeOrder response.
...
def _normalize_tx_item_for_signing(tx_item: dict) -> tuple[dict, int]:
    """
    Normalize makeOrder tx item to { to, calldata, gasLimit, nonce, gasPrice, value } and chainId.
    ...
    if cid == _SOLANA_CHAIN_ID:
        raise ValueError(
            "Tx item is Solana (chainId 501). Use --private-key-file-sol; order should be detected as Solana."
        )
    ...
    if derive and isinstance(data_raw, str):
        ...
        return tx_data, cid
    # Legacy format: tx_item["data"] is dict
...
def sign_order_txs_evm(order_data: dict, private_key: str, chain_id: int = None) -> list[str]:
    """
    Sign all EVM transactions in a makeOrder txs response.
    Supports:
      1. Legacy format (tx_item["data"] is dict)
    ...
    for tx_item in txs:
        ...
        if item_cid == _SOLANA_CHAIN_ID:
            raise ValueError(
                "One or more tx items are Solana (chainId 501). Use --private-key-file-sol for Solana orders."
            )
...
def sign_order_txs_tron(order_data: dict, private_key_tron: str) -> list[str]:
    ...
    for tx_item in txs:
        ...
        if chain not in ("trx", "tron"):
            raise ValueError(
                f"Tx chain is {chain!r}, not Tron. Use --private-key-file-tron only for chain trx/tron."
            )
...
def main():
    parser = argparse.ArgumentParser(description="Sign makeOrder (make-order) response")
    parser.add_argument("--order-json", help="makeOrder response JSON string")
    ...
    if "signatures" in data and data["signatures"]:
        if not args.private_key:
            print("ERROR: --private-key-file required for EVM signatures mode", file=sys.stderr)
            sys.exit(1)
    ...
    if "txs" in data and data["txs"]:
        if _is_solana_order(data):
            ...
            if not pk_sol:
                print("ERROR: --private-key-file-sol required for Solana txs mode", file=sys.stderr)
                sys.exit(1)
            signed = sign_order_txs_solana(data, pk_sol)
        elif _is_tron_order(data):
            pk_tron = args.private_key_tron
            if not pk_tron:
                print("ERROR: --private-key-file-tron required for Tron txs mode", file=sys.stderr)
                sys.exit(1)
            signed = sign_order_txs_tron(data, pk_tron)
        else:
            if not args.private_key:
                print("ERROR: --private-key-file required for EVM txs mode", file=sys.stderr)
                sys.exit(1)
