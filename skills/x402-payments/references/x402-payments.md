...
### Permit2 (Protocol-Defined, Not Yet Implemented)

> **Note:** Permit2 is defined in the x402 protocol spec as a universal fallback for tokens without EIP-3009. However, `scripts/x402_pay.py` does **not** implement Permit2 signing. If a server requests Permit2, the script will raise `NotImplementedError`. In practice, x402 is USDC-first and EIP-3009 covers all current use cases.
