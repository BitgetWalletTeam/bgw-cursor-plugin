...
**Signing flow:** After makeOrder, derive the private key from mnemonic in secure storage, write it to a **unique** temporary file programmatically (`tempfile.mkstemp`, never shell `echo`), pass the key file to `order_sign.py` (which reads and deletes the file). The Agent/CLI flow always uses `data.txs`: the script signs each tx item, you fill `txs[].sig`, then send. For EVM gasPayMaster orders, the signing output is a JSON string of the `msgs[]` array with `sig` fields filled (still placed in `txs[].sig`). MakeOrder unsigned data expires in ~60 seconds — sign and send must follow immediately. **Never pass private keys as command-line arguments** (they are visible in `ps` and shell history).

> **Note:** The partner Swap Order API (`bopenapi`) may return `data.signatures` instead of `data.txs` for certain gasless flows. That model is documented in `skills/api-debugging/references/swap-order.md` and is not part of the Agent/CLI flow described here.
...
| 4′ | `order_sign.py` → fill `txs[].sig` | Sign `data.txs` items (EVM regular/gasPayMaster msgs[], Solana, Tron) |
...
- **Script (Tron):** `python3 scripts/order_make_sign_send.py --private-key-file-tron /tmp/.pk_tron --from-address <tron_addr> --to-address <tron_addr> --order-id <from_confirm> --from-chain trx ...`
- **Behavior:** Takes private key from secure storage, calls makeOrder, signs `data.txs`, fills `txs[].sig`, then sends. Auto-detects EVM, Solana, or Tron from makeOrder response. Never outputs private keys. Use this so the ~60s makeOrder expiry does not run out.
...
- **Sign:** Derive private key from mnemonic in secure storage. Write to a unique temp file programmatically (`tempfile.mkstemp`). Pass full makeOrder response to `order_sign.py` (stdin or `--order-json`) with the appropriate key flag: `--private-key-file <path>` (EVM), `--private-key-file-sol <path>` (Solana), or `--private-key-file-tron <path>` (Tron). The script reads the key, deletes the file, auto-detects the chain/mode within `data.txs`, signs, and outputs a JSON array. Output format varies by chain:
  - **EVM regular tx**: `0x`-prefixed signed tx hex
  - **EVM gasPayMaster `msgs[]`**: JSON string of the msgs array with `sig` fields filled
  - **Solana**: base58-encoded signed transaction
  - **Tron**: JSON string with `{ signature, txID, raw_data }`
- **Fill & send:** Set `data.txs[i].sig` from the output array, then `bitget-wallet-agent-api.py send --json-stdin` or `--json-file` with body `{ "orderId": data.orderId, "txs": data.txs }`.

### 6. Query order (getOrderDetails / getSwapOrder)
...
- **getOrderDetails** (CLI) / **getSwapOrder** (partner API — same function, different path): `data.details.status === "success"` means success; show `fromTxId` / `toTxId` and block explorer links. When status is success, ignore the response `tips` field. When status is not success, use the `tips` field to prompt the user and suggest retry or adjusting amount/slippage if appropriate.

## Supported swap chains

The swap API supports 8 chains. Use these chain codes in all swap commands:
...
| Tron | trx | `https://tronscan.org/#/transaction/{txId}` |
