...
`scripts/order_sign.py` handles signing for the makeOrder → send flow.
...
# Tron: use --private-key-file-tron
...
| `data.txs` + chain=trx\|tron or `transaction.raw_data_hex` present | Tron | `sign_order_txs_tron()` (secp256k1 like EVM, but Tron-specific handler and derivation path) |
...
| Tron | `eth-account` (same secp256k1 signing as EVM) |
