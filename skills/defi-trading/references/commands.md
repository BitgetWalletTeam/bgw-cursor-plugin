...
| `kline` | Get K-line (OHLC) for a token. `--period` 1s,1m,5m,15m,30m,1h,4h,1d,1w; `--size` max 1440. | User asks for K-line / chart data. See `skills/token-analysis/references/market-data.md`. |
...
| `security` | Security audit (highRisk, riskCount, buyTax/sellTax, etc.). | Before swap for unfamiliar tokens; user asks for safety check. See `skills/token-analysis/references/market-data.md`. |
...
One-shot makeOrder + sign + send. Supports EVM, Solana, and Tron. Auto-detects chain from makeOrder response.
...
# Tron
python3 scripts/order_make_sign_send.py --private-key-file-tron /tmp/.pk_tron --from-address <tron_addr> --to-address <tron_addr> --order-id <from_confirm> --from-chain trx --from-contract <contract> --from-symbol USDT --to-chain trx --to-contract "" --to-symbol TRX --from-amount 10 --slippage 1.00 --market ... --protocol ...
```
...
Sign order/makeOrder transaction data. Takes makeOrder response JSON and auto-detects the chain within `data.txs`. Outputs a JSON array of signed strings to fill into `txs[].sig`. Output format varies by chain — EVM regular: 0x-prefixed signed tx hex; EVM gasPayMaster msgs[]: JSON string of msgs with sig fields filled; Solana: base58 signed tx; Tron: JSON with signature/txID/raw_data. Also supports `data.signatures` mode (partner API EIP-712 hashes) if present.

Supports: EVM raw tx signing, EVM gasPayMaster (gasless msgs eth_sign), EIP-712 typed data, Solana Ed25519, Solana gasPayMaster (partial-sign on serializedTransaction), and Tron (secp256k1, Tron-specific handler).
...
# Solana
echo '<makeOrder_json>' | python3 scripts/order_sign.py --private-key-file-sol /tmp/.pk_sol
# Tron
echo '<makeOrder_json>' | python3 scripts/order_sign.py --private-key-file-tron /tmp/.pk_tron
```
...
All subcommands under `scripts/bitget-wallet-agent-api.py`. See [`skills/token-analysis/references/token-analyze.md`](../../token-analysis/references/token-analyze.md) for full domain knowledge.
...
All subcommands under `scripts/bitget-wallet-agent-api.py`. See [`skills/token-analysis/references/address-find.md`](../../token-analysis/references/address-find.md) for full domain knowledge.
